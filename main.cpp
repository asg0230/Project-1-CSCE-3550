...
// Encodes data into a URL-safe base64 string.
...
int main()
{
    // Initialize and configure the RSA key pair.
    ...
    svr.Get("/.well-known/jwks.json", [&](const httplib::Request &, httplib::Response &res)
            {
                // Extract RSA parameters n and e.
    ...
    // Catch-all handler for unsupported methods on known routes.
